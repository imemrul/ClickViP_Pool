...
<table border="0" cellpadding="0" cellspacing="0">
    ...
    <tr>
        <td>
            ...
            <span style="color:#d95e46;text-transform:uppercase;font-size:12px">DATE: {!! Carbon\Carbon::now()->startOfWeek()->toDateString() !!} - {!! Carbon\Carbon::now()->endOfWeek()->toDateString() !!}</span>
            @if(sizeof($activities) == 0)
                <h4>No summary was found.</h4>
            @endif
            @foreach($activities as $index=>$activity)
                <h4 style="margin:0px 0px;padding: 5px 3px;background:#00987717">{!! $activity->first()->client->name or 'Unknown client' !!}</h4>
                <?php $counter = 1; ?>
                @foreach($activity as $row)

                    <p style="padding:0;font-weight:700;font-size:12px;line-height:11px;margin:6px 0 0;color:#009877">
                        {!! $counter !!}. {!! $row->title !!} <span style="font-style: italic; font-size:11px;color:#424e4e;font-weight:300"> - {!! $row->executive->full_name !!}</span>
                    </p>
    ...
    <tr>
        <td colspan="2" class="text-center">
            <a href="{!! URL::to('module/activity') !!}">Back to CRM</a>
        </td>
    </tr>
    </tfoot>
</table>
